public class Solution{
    ...
    Some people may come up with idea O(KN^2) where dp[K][N] = 1 + max(dp[K - 1][i - 1], dp[K][N - i]) for i in 1...N.
    However, this idea is very brute force, for the reason that you check all possiblity.
    
    
    */

    public in eggDrop(int K, int N){
        int[][] dp = new int[N+1][K+1];
        int m = 0; // movement
        while(dp[m][K] < N){
            ++m;
            for(int k =1; k<=K; k++){
                dp[m][k] = dp[m-1][k-1]+dp[m-1][k] +1; //(for current floor)
            }
        }
        return m;
    }
